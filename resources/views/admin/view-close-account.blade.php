...
<section id="main-content" class=" ">
    ...
    <div class='col-xs-12'>
        <div class="page-title">
            ...
            <div class="pull-left">
                <!-- PAGE HEADING TAG - START --><h1 class="title">View All Close Account</h1><!-- PAGE HEADING TAG - END -->                            </div>
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li>
                            <a href="{{ url('admin/view-close-account') }}">View All Close Account</a>
                        </li>
...
<div class="col-lg-12">
    <section class="box ">
            <header class="panel_header">
                <h2 class="title pull-left">View All Close Account</h2>
...
@if(Session::has('error'))
            ...
            <table class="table table-striped" >
                ...
                <tbody>
                @foreach ($closeaccount as $row)
                    <tr>
                     	<td >{{ $row->close_id }}</td>
                     	<td >{{ $row->firstname.' '.$row->lastname }}</td>
                     	<td >{{'+'.$row->mobile }}</td>
                        <td >{{ $row->close_reason }}</td>
                        <td>{{ $row->bad_service }}</td>
                            <td>{{ $row->other_reason }}</td>
                        
                        
                        <td>
                        <td>
                        <a href="{{ url('admin/delete-close-account',$row->close_id) }}" onclick="return confirm('Are you sure you want to delete this record ?');" ><i class="fa fa-archive" title="delete"></i></a>
                        </td>
